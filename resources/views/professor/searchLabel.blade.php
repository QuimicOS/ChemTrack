...
<script>
  ...
  function getStatusText(status_of_label) {
    switch (status_of_label) {
        case 0:
            return { text: 'Invalid', backgroundColor: '#ff4d4d', textColor: 'white' }; // Softer red
        case 1:
            return { text: 'Pending', backgroundColor: '#ffeb3b', textColor: 'black' }; // Softer yellow
        case 2:
            return { text: 'Completed', backgroundColor: '#4caf50', textColor: 'white' }; // Green
        default:
            return { text: 'Unknown', backgroundColor: '#bdbdbd', textColor: 'black' }; // Gray
    }
}
...
document.getElementById('searchButton').addEventListener('click', function () {
    ...
    fetch(`/Proflabel/${labelID}`)
        ...
        .then(data => {
            ...
            // Set label status text and colors
            const statusElement = document.getElementById('status');
            const statusInfo = getStatusText(data.status_of_label);
            statusElement.value = statusInfo.text;
            statusElement.style.backgroundColor = statusInfo.backgroundColor;
            statusElement.style.color = statusInfo.textColor;
        ...
        .catch(error => {
            ...
            alert('Label not found or access denied to view this label.');
        });
});
...
document.getElementById('searchRoomButton').addEventListener('click', function () {
    ...
    fetch(`/Proflabel/room/${roomNumber}`)
        .then(response => {
            if (!response.ok) {
